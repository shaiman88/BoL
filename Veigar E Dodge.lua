local FoundVeigar = false
for i, enemy in ipairs(GetEnemyHeroes()) do
	if enemy and enemy.charName == "Veigar" then FoundVeigar = true; break end
end

if not FoundVeigar then return end
...
local MyHitBox = (hitboxes[myHero.charName] ~= nil and hitboxes[myHero.charName] ~= 0) and hitboxes[myHero.charName] or 65
local MyHitBoxSqr = MyHitBox * MyHitBox
--Credit to Honda for hitboxes (from VPrediction)

function OnLoad()
	Config = scriptConfig("Veigar E Dodge", "veigaredodge")
	Config:addParam("dodge", "Enable dodge", SCRIPT_PARAM_ONKEYTOGGLE, true, GetKey("M"))
	print("<font color='#A90BDE'>Veigar </font><font color='#FFFFFF'>E dodge loaded.</font>")
end

function OnProcessSpell(unit, spell)
	if spell.name == "VeigarEventHorizon" and unit.team ~= myHero.team then
		VeigE = {Position = {x = spell.endPos.x, z = spell.endPos.z}, EndTime = os.clock()+3}
	end
end

function OnTick()
	if myHero.dead or not Config.dodge then return end

	if VeigE ~= nil then
		if os.clock() > VeigE.EndTime then
			VeigE = nil
		else
			local HeroVector = Vector(myHero.visionPos.x, 0, myHero.visionPos.z)
			local RingVector = Vector(VeigE.Position.x, 0, VeigE.Position.z)

			local pnt = RingVector + (HeroVector-RingVector):normalized()*350
			local DistFromRing = GetDistanceSqr(HeroVector, pnt)

			if DistFromRing < MyHitBoxSqr + 24000 then
				local DodgePosition
				local DistFromCenterRing = GetDistanceSqr(HeroVector, RingVector)
				if DistFromCenterRing < 122500 then 
					--[[Inside of ring]]
					DodgePosition = HeroVector + (RingVector-HeroVector):normalized()*60
				else
					--[[Outside of ring]]
